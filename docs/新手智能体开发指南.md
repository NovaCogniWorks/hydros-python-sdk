...
class MyFirstAgent(CustomAgent):
    ...
    def on_terminate(
        ...
        runtime.logger.info("终止智能体: %s", runtime.agent.agent_id)
```
...
CustomAgent
    ↓ callbacks receive
AgentExecutionContext + protocol request DTO
```

新增 Agent 默认实现 `CustomAgent`。协议请求和响应从 `hydros_agent_sdk.protocol` 导入；SDK 内建的仿真、调度等高级基类位于 `hydros_agent_sdk.agents`，不属于根包公共面。
...
def on_init(self, request: SimTaskInitRequest) -> SimTaskInitResponse:
    ...
    # 4. 返回成功响应；task/runtime 登记由 MultiAgentCallback 统一完成
...
def on_terminate(self, request: SimTaskTerminateRequest) -> SimTaskTerminateResponse:
    ...
    # 2. 返回成功响应；task/runtime 清理由 MultiAgentCallback 统一完成
...
SDK 自动支持多任务：每个任务会创建独立的智能体实例。

```python
# 工厂模式自动处理
agent_factory = CustomAgentFactory(
    custom_agent_class=MyAgent,
    config_file=CONFIG_FILE,
    env_config=env_config
)
...
```python
# Agent 身份
agent_code = runtime.agent.agent_code
agent_type = runtime.agent.agent_type

# Agent 配置
topology_url = runtime.config.get_property('hydros_objects_modeling_url')
time_step = runtime.config.get_property('time_step', default=60)
```
...
- [ ] 实现了根包公开的 `CustomAgent`
- [ ] 按需要实现 `on_init()`、`on_tick()`、`on_terminate()` 等回调
- [ ] 协议 DTO 从 `hydros_agent_sdk.protocol` 导入，未依赖内部 runtime/state 对象
- [ ] 添加了错误处理（装饰器或上下文管理器）
...
# 使用结构化日志
runtime.logger.info(
    "初始化智能体: agent_id=%s, task_id=%s",
    runtime.agent.agent_id,
    runtime.simulation_context.biz_scene_instance_id,
)
runtime.logger.debug("边界条件: %d 个对象", len(boundary_conditions))
runtime.logger.error("仿真失败: step=%s", step, exc_info=True)
```
...
- 从 `CustomAgent` 和 `examples/agents/template` 开始
- 先实现最简单的版本，再逐步添加功能
- 充分利用错误处理机制，让代码更健壮
