...
show_help() {
    ...
    echo ""
    echo "示例:"
    echo "  $0 scheduling               # 启动泵站中央调度智能体"
    echo "  $0 --all                    # 启动所有 agents"
    echo "  $0 --logs                   # 查看日志"
    echo "  $0 --debug scheduling       # 启用调试模式启动泵站中央调度智能体"
    echo "  $0 --enable-system-central-scheduling-agent scheduling"
    echo "                               # 注册 SDK 内置中央调度，并启动默认泵站 agents"
    echo "  $0 --debug --debug-nowait scheduling  # 调试模式但不等待调试器"
    echo ""
    echo "调试模式:"
