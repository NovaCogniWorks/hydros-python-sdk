...
export DOCKER_HOST="${DOCKER_HOST:-tcp://192.168.1.24:2375}"
VERSION="${VERSION:-v1.0.0}"

IMAGE_NAME="${IMAGE_NAME:-hydros-twins-agent}"
CONTAINER_NAME="${CONTAINER_NAME:-hydros-twins-agent}"
...
START_ARGS="${START_ARGS:-twins}"
PORT="${PORT:-8015}"
LOG_VOLUME="${LOG_VOLUME:-${CONTAINER_NAME}-logs}"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"
TWINS_DIR="${REPO_ROOT}/custom-agent/twins"

docker build \
    -f "${TWINS_DIR}/Dockerfile" \
...
docker run -d \
    ...
    -v "${LOG_VOLUME}:/opt/hydros/custom-agent/twins/logs" \
    "${IMAGE_NAME}:${VERSION}" \
