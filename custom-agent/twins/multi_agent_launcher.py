...
def resolve_agent_dir(agent_name: str) -> Optional[str]:
    """Resolve an agent directory for both flat and nested example layouts."""
    candidates = [
        os.path.join(EXAMPLES_DIR, 'agents', agent_name),
        EXAMPLES_DIR if os.path.basename(EXAMPLES_DIR) == agent_name else None,
    ]

    for candidate in candidates:
        if candidate and os.path.isdir(candidate):
            props_file = os.path.join(candidate, 'agent.properties')
...
def discover_all_agents() -> List[str]:
    ...
    flat_props_file = os.path.join(EXAMPLES_DIR, 'agent.properties')
    if os.path.exists(flat_props_file):
        available_agents.append(os.path.basename(EXAMPLES_DIR))

    agents_dir = os.path.join(EXAMPLES_DIR, 'agents')
    if not os.path.exists(agents_dir):
        return sorted(set(available_agents))
...
class MultiAgentCoordinator:
    ...
    def load_agent_module(self, agent_name: str) -> Dict[str, Any]:
        ...
        """
        # 1. Resolve agent directory for both flat and nested layouts.
        agent_dir = resolve_agent_dir(agent_name)
        if agent_dir is None:
            raise ValueError(
                f"Agent directory not found for '{agent_name}'. "
                f"Checked: {os.path.join(EXAMPLES_DIR, 'agents', agent_name)} and {EXAMPLES_DIR}"
            )
