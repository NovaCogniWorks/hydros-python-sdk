"""
HydroSim_v47.py - 梯级水电站联合仿真
============================================================
目标：
1. 参数化生成来水与总出力指令曲线。
2. 站间出力分配以总出力零误差为硬约束，先按四站单机额定出力比例快速分配，再按耗水量平衡微调。
3. 水库泄洪使用 PID + 动态前馈过流。
4. 2/3/4 级水库低水位保护优先：黄区半额限出力，红区全额限出力；若出现总出力短缺，先解除黄区限制，再解除红区限制。
5. 红黄区保护判定完成后，剩余出力按单机额定出力比例分配，并用快速耗水估算做微调。
6. 导出正式仿真 CSV、运行摘要、调度最小单位配置、图像和 Markdown 报告。
7. V37：站间同时平衡发电流量和水库入流-发电流量缺口，下游低水位效率系数在 0.9~1.0 连续调整。
8. V38：清理旧站间分配支路，保留连续分配、慢重平衡、低水位修正和站内连续经济运行。
9. V39：后三站低水位效率折减量回补到瀑布沟，增强上游库存承担出力的倾向。
10. V40：将瀑布沟效率回补强度从 1.0 降到 0.6，平衡水位保护和站间流量一致性。
11. V41：站间慢重平衡按流量步长动态折算 MW 调整量，削弱低水头电站的流量台阶。
12. V42：站内到达当前在线机组额定满发即提前开下一台，避免顶到物理上限才切换。
13. V43：清理旧站内迟滞开机函数和未使用参数，保留当前组合带宽与提前开机主路径。
14. V44：以 V43 为基础，替换为 V100 站间分配算法。
15. V45：保留 V44 的基础连续分配、慢重平衡和总出力闭合，将 PID 水位修正层替换为轻量 MPC 修正层。
16. V46：在保持 V45 数值行为的基础上，公开信号生成、组件创建、阶段运行、历史重置和结果汇总函数。
17. V47：公开站内/站间分配接口，并统一水位 PID 参数入参。
"""
...
class HydroUnit:
    ...
    def step(self) -> None:
        ...
        self.current_power = _clip(self.current_power, 0.0, self.max_power)
...
class HydroStation:
    ...
    def allocate_power(self, target_power: float, update_commitment_memory: bool = False) -> Dict[str, object]:
        """站内分配接口：按最少耗水策略分配到机组并返回分配结果。"""
        self.target_p = float(target_power)
        self._dispatch_units_equal_increment(update_commitment_memory=update_commitment_memory)
        indices = np.flatnonzero(self.unit_array[:, 0] > 1e-9).astype(int)
        return {
            "station": self.name,
            "target_power_mw": float(self.target_p),
            "unit_indices": indices.tolist(),
            "unit_power_mw": self.unit_array[indices, 0].astype(float).tolist(),
            "estimated_flow_m3s": float(sum(self._unit_flow_for_power(self.multi_station[int(self.unit_array[i, 5])], self.unit_array[i, 0]) for i in indices)),
        }
...
class HydroStair:
    ...
    def __init__(
        ...
        pid_parameters: PIDParameters | None = None,
    ):
        ...
        if pid_parameters is not None:
            self.configure_pid_parameters(pid_parameters)
    ...
    def configure_pid_parameters(self, params: PIDParameters) -> None:
        """统一设置梯级水位 PID 参数；数组长度必须等于电站数。"""
        fields = ("kp_mw_per_m", "ki_mw_per_m_step", "kd_mw_per_m", "max_delta_p", "output_limit_p", "forecast_full_deficit_m3s")
        for field in fields:
            values = tuple(getattr(params, field))
            if len(values) != self.num_stairs:
                raise ValueError(f"PID 参数 {field} 长度必须为 {self.num_stairs}。")
        self.stage_pid_enable = bool(params.enable)
        self.stage_pid_deadband_m = float(params.deadband_m)
        self.stage_pid_kp_mw_per_m = np.asarray(params.kp_mw_per_m, dtype=float)
        self.stage_pid_ki_mw_per_m_step = np.asarray(params.ki_mw_per_m_step, dtype=float)
        self.stage_pid_kd_mw_per_m = np.asarray(params.kd_mw_per_m, dtype=float)
        self.stage_pid_max_delta_p = np.asarray(params.max_delta_p, dtype=float)
        self.stage_pid_output_limit_p = np.asarray(params.output_limit_p, dtype=float)
        self.stage_pid_forecast_min_scale = float(params.forecast_min_scale)
        self.stage_pid_forecast_full_deficit_m3s = np.asarray(params.forecast_full_deficit_m3s, dtype=float)
        self.stage_pid_integral_limit_m_step = float(params.integral_limit_m_step)
    ...
    def allocate_power(self, total_power: float, stage_hints: Sequence[Dict] | None = None) -> Dict[str, object]:
        """站间分配接口：将总出力按水量平衡和水位保护分配到各电站。"""
        if stage_hints is not None:
            self.update_stage_hints(stage_hints)
        self.total_p_target = float(total_power)
        self._station_dispatch()
        return {
            "target_power_mw": float(self.total_p_target),
            "station_power_mw": self.stair_array[:, 2].astype(float).tolist(),
            "station_flow_m3s": self.stair_array[:, 3].astype(float).tolist(),
            "total_power_mw": float(self.total_p_current),
            "total_flow_m3s": float(self.flow),
        }
...
class PIDParameters:
    """统一的水位 PID 参数结构；数组参数按电站顺序提供。"""

    enable: bool = True
    deadband_m: float = 0.2
    kp_mw_per_m: Sequence[float] = ()
    ki_mw_per_m_step: Sequence[float] = ()
    kd_mw_per_m: Sequence[float] = ()
    max_delta_p: Sequence[float] = ()
    output_limit_p: Sequence[float] = ()
    forecast_min_scale: float = 0.3
    forecast_full_deficit_m3s: Sequence[float] = ()
    integral_limit_m_step: float = 20.0

    def __init__(self, enable=True, deadband_m=0.2, kp_mw_per_m=(), ki_mw_per_m_step=(),
                 kd_mw_per_m=(), max_delta_p=(), output_limit_p=(), forecast_min_scale=0.3,
                 forecast_full_deficit_m3s=(), integral_limit_m_step=20.0):
        self.enable = bool(enable)
        self.deadband_m = float(deadband_m)
        self.kp_mw_per_m = tuple(kp_mw_per_m)
        self.ki_mw_per_m_step = tuple(ki_mw_per_m_step)
        self.kd_mw_per_m = tuple(kd_mw_per_m)
        self.max_delta_p = tuple(max_delta_p)
        self.output_limit_p = tuple(output_limit_p)
        self.forecast_min_scale = float(forecast_min_scale)
        self.forecast_full_deficit_m3s = tuple(forecast_full_deficit_m3s)
        self.integral_limit_m_step = float(integral_limit_m_step)

    @classmethod
    def from_station_configs(cls, station_cfgs: Sequence[Dict]) -> "PIDParameters":
        """从旧版逐站配置字典构造统一参数。"""
        if not station_cfgs:
            raise ValueError("station_cfgs 不能为空。")
        first = station_cfgs[0]
        return cls(
            enable=bool(first.get("stage_pid_enable", True)),
            deadband_m=float(first.get("stage_pid_deadband_m", 0.2)),
            kp_mw_per_m=tuple(float(sc.get("stage_pid_kp_mw_per_m", 0.0)) for sc in station_cfgs),
            ki_mw_per_m_step=tuple(float(sc.get("stage_pid_ki_mw_per_m_step", 0.0)) for sc in station_cfgs),
            kd_mw_per_m=tuple(float(sc.get("stage_pid_kd_mw_per_m", 0.0)) for sc in station_cfgs),
            max_delta_p=tuple(float(sc.get("stage_pid_max_delta_p", 0.0)) for sc in station_cfgs),
            output_limit_p=tuple(float(sc.get("stage_pid_output_limit_p", sc.get("stage_pid_max_delta_p", 0.0))) for sc in station_cfgs),
            forecast_min_scale=float(first.get("stage_pid_forecast_min_scale", 0.3)),
            forecast_full_deficit_m3s=tuple(float(sc.get("stage_pid_forecast_full_deficit_m3s", 200.0)) for sc in station_cfgs),
            integral_limit_m_step=float(first.get("stage_pid_integral_limit_m_step", 20.0)),
        )
...
POWER_CONFIGS = [
    {
        ...
        "min_power": 200,
        "max_power": 3900,
        "stair_min_step_p": 50.0,
        "unit_min_step_p": 30.0,
        "station_target_ramp_rate": 180.0,
        "flow_rebalance_deadband_m3s": 100.0,
        "low_stage_pubugou_priority_spread_tolerance_m3s": 0.0,
        "low_stage_factor_start_m": 0.2,
        "low_stage_factor_full_m": 1.0,
        "inflow_deficit_balance_gain": 0.35,
        "inflow_deficit_balance_deadband_m3s": 10.0,
        "flow_rebalance_max_delta_p": 50.0,
        "slow_rebalance_max_delta_p": 20.0,
        "slow_rebalance_max_delta_q_m3s": 20.0,
        "stage_pid_enable": True,
        "stage_pid_deadband_m": 0.2,
        "stage_pid_integral_limit_m_step": 10.0,
        "stage_pid_forecast_min_scale": 0.3,
        "stage_mpc_enable": True,
        "stage_mpc_horizon_steps": 8,
        "stage_mpc_stage_weight": 18.0,
        "stage_mpc_forecast_weight": 4.0,
        "stage_mpc_flow_spread_weight": 0.002,
        "stage_mpc_move_weight": 0.08,
        "stage_mpc_min_improve": 1e-5,
        "stage_mpc_candidate_fracs": (0.25, 0.5, 1.0),
    },
    {
        "ID": 2,
        "Name": "深溪沟",
        ...
        "min_power": 60,
        "max_power": 726,
        "stair_min_step_p": 10.0,
        "unit_min_step_p": 8.25,
        "station_target_ramp_rate": 50.0,
        "flow_rebalance_max_delta_p": 10.0,
        "slow_rebalance_max_delta_p": 3.0,
        "slow_rebalance_max_delta_q_m3s": 3.0,
        "inflow_deficit_balance_deadband_m3s": 60.0,
        "low_stage_inter_station_efficiency_factor": 0.9,
        "stage_pid_kp_mw_per_m": 2.0,
        "stage_pid_ki_mw_per_m_step": 0.02,
        "stage_pid_kd_mw_per_m": 0.0,
        "stage_pid_max_delta_p": 2.0,
        "stage_pid_output_limit_p": 1.2,
        "stage_pid_forecast_full_deficit_m3s": 180.0,
    },
    {
        "ID": 3,
        "Name": "枕头坝I期",
    ...
    },
    {
        ...
        "stair_min_step_p": 10.0,
        "unit_min_step_p": 2.9,
        "station_target_ramp_rate": 30.0,
        "flow_rebalance_max_delta_p": 10.0,
        "slow_rebalance_max_delta_p": 1.0,
        "slow_rebalance_max_delta_q_m3s": 1.5,
        "inflow_deficit_balance_deadband_m3s": 30.0,
        "low_stage_inter_station_efficiency_factor": 0.9,
        "stage_pid_kp_mw_per_m": 0.8,
        "stage_pid_ki_mw_per_m_step": 0.01,
        "stage_pid_kd_mw_per_m": 0.0,
        "stage_pid_max_delta_p": 0.5,
        "stage_pid_output_limit_p": 0.25,
        "stage_pid_forecast_full_deficit_m3s": 50.0,
    },
]
    ...
    [
        {
            ...
            "design_power": 600.0,
            "max_power": 660.0,
            "power_ramp_rate": 80.0,
            "design_efficiency": 0.92,
            "eta_head_coeff": 0.25,
            "eta_power_coeff": 0.40,
        }
        for k in range(1, 7)
    ],
    [
...
def generate_input_signals(
    ...
    """生成完整仿真周期的来流和总出力指令数组。"""
    if sim_steps <= 0 or warm_steps <= 0:
        raise ValueError("sim_steps 与 warm_steps 必须为正整数。")
    flow_low, flow_high = _validate_range("flow_range", *flow_range)
    power_low, power_high = _validate_range("power_range", *power_range)
    total = int(sim_steps) + int(warm_steps)
    flow = NormalizedSignal(total, [1200, 300, 770, 6000], seed=flow_seed).scale(flow_low, flow_high)
    power = NormalizedSignal(total, [1200, 300, 770, 6000], seed=power_seed).scale(power_low, power_high)
    return flow.signal.copy(), power.signal.copy()


def create_simulation_components(
    sim_steps: int, warm_steps: int, power_signal: Sequence[float],
    pid_parameters: PIDParameters | None = None,
) -> Dict[str, object]:
    """按 V46 默认配置创建河道、水库群和电站梯级对象。"""
    _validate_configs()
    if len(power_signal) < sim_steps + warm_steps:
        raise ValueError("power_signal 长度不足以覆盖预热和正式阶段。")
    return {
        "river": RiverArray(1, "大渡河_水力", FLOW_CONFIGS, max(sim_steps, warm_steps), CAPA_LOC),
        "reservoir": HydroResStairs(1, "大渡河_水库", FLOW_CONFIGS, FLOW_STATION_CFGS, CAPA_LOC),
        "stair": HydroStair(1, "大渡河_电站", float(power_signal[0]), POWER_CONFIGS, UNIT_CONFIGS, pid_parameters=pid_parameters),
    }
...
def run_simulation_phase(
    ...
    idx_start: int,
    idx_end: int,
    flows_in: Sequence[float],
    power_cmd: Sequence[float],
    components: Dict[str, object],
    progress_interval: int = 100,
) -> None:
    """执行一个仿真阶段；components 来自 create_simulation_components。"""
    _run_phase(title, idx_start, idx_end, progress_interval, np.asarray(flows_in), np.asarray(power_cmd),
               components["river"], components["reservoir"], components["stair"])


def reset_simulation_histories(components: Dict[str, object]) -> None:
    """清空阶段历史记录，使后续记录成为独立正式阶段。"""
    components["river"].history_reset()
    components["reservoir"].history_reset()
    components["stair"].history_reset()


def summarize_simulation(components: Dict[str, object]) -> List[Dict]:
    """返回各水库 Green/Yellow/Red 分区统计。"""
    return _stage_zone_summary(components["reservoir"])


def allocate_intra_station_power(station: HydroStation, target_power_mw: float, update_commitment_memory: bool = False) -> Dict[str, object]:
    """站内分配公共接口。"""
    return station.allocate_power(target_power_mw, update_commitment_memory)


def allocate_inter_station_power(stair: HydroStair, total_power_mw: float, stage_hints: Sequence[Dict] | None = None) -> Dict[str, object]:
    """站间分配公共接口。"""
    return stair.allocate_power(total_power_mw, stage_hints)


def _csv_token(name: str) -> str:
...
def _stage_zone_summary(multi_reservoir: HydroResStairs) -> List[Dict]:
    summary: List[Dict] = []
...
def _export_formal_results_csv(
    output_dir: str,
    flows_in: np.ndarray,
    power_cmd: np.ndarray,
    warm_steps: int,
    ...
    for i in range(multi_stair.num_stairs):
        ...
        for u in sta.multi_station:
            if len(u.history["current_power"]) != formal_steps:
                raise ValueError(f"{sta.name}/{u.unit_name}: 机组历史步数与正式仿真步数不一致。")

    station_infos = []
    for i, sta in enumerate(multi_stair.multi_stair):
        st_token = f"st{i + 1}_{_csv_token(sta.name)}"
        station_infos.append(
            {
                "station": sta,
                "reservoir": multi_reservoir.Capacity_Stairs[i],
                "station_token": st_token,
                "station_p_hist": sta.history["current_power"],
                "station_q_hist": sta.history["flow"],
                ...
                "units": [
                    {
                        "unit_token": f"u{j + 1}_{_csv_token(u.unit_name)}",
                        "p_hist": u.history["current_power"],
                        "q_hist": u.history["flow"],
                        "n_hist": u.history["efficiency"],
                    }
                    for j, u in enumerate(sta.multi_station)
                ],
            }
        )

    csv_path = os.path.join(output_dir, "formal_results_v47.csv")
    with open(csv_path, "w", newline="", encoding="utf-8-sig") as f:
        ...
        header = [
            ...
            "global_step",
            "upstream_inflow_m3s",
            "total_dispatch_cmd_mw",
        ]

        for info in station_infos:
            st = info["station_token"]
            header.extend([
                f"{st}_station_p_mw",
                f"{st}_station_q_m3s",
                f"{st}_spill_qout_m3s",
                f"{st}_reservoir_stage_m",
                f"{st}_stage_delta_m",
                f"{st}_stage_zone",
                f"{st}_stage_direction",
            ])

        for info in station_infos:
            st = info["station_token"]
            for u_info in info["units"]:
                ut = u_info["unit_token"]
                header.extend([
                    f"{st}_{ut}_p_mw",
                    f"{st}_{ut}_q_m3s",
                    f"{st}_{ut}_n_efficiency",
                ])
        ...
        for k in range(formal_steps):
            ...
            row = [
                k,
                global_idx,
                float(flows_in[global_idx]),
                float(power_cmd[global_idx]),
            ]
            ...
            for info in station_infos:
                for u_info in info["units"]:
                    row.extend([
                        float(u_info["p_hist"][k]),
                        float(u_info["q_hist"][k]),
                        float(u_info["n_hist"][k]),
                    ])
...
def _export_run_summary_json(
    ...
    path = os.path.join(output_dir, "run_summary_v47.json")
...
def _export_simulation_report_md(
    output_dir: str,
    sim_steps: int,
    warm_steps: int,
    flow_seed: int,
    ...
    report_path = os.path.join(output_dir, "simulation_report_v47.md")
    generated_at = datetime.datetime.now().isoformat(timespec="seconds")
    ...
    for row in stage_zone_summary:
        ...
        lines.append(
            ...
            f"{_format_pct(green, total)} | {_format_pct(yellow, total)} | {_format_pct(red, total)} | {total} |"
        )
    ...
    lines.extend(
        [
            ...
            "## 整体分区结论",
            "",
            f"- 正式仿真共统计 {total_zone_points} 个水库分区点。",
            f"- Green：{total_green} 点（{_format_pct(total_green, total_zone_points)}）。",
            f"- Yellow：{total_yellow} 点（{_format_pct(total_yellow, total_zone_points)}）。",
            f"- Red：{total_red} 点（{_format_pct(total_red, total_zone_points)}）。",
            "",
            "## 水库运行指标",
            "",
            "| 水库 | 水位最小值(m) | 水位最大值(m) | 水位平均值(m) | 末步水位(m) | 泄洪最大值(m^3/s) | 发电出流平均值(m^3/s) |",
            "|---|---:|---:|---:|---:|---:|---:|",
        ]
    )

    for res in multi_reservoir.Capacity_Stairs:
        stage = _series_stats(res.history["current_stage"])
        spill = _series_stats(res.history["current_outflow_discharge"])
        power_q = _series_stats(res.history["current_outflow_power"])
        lines.append(
            f"| {res.name} | {stage['min']:.3f} | {stage['max']:.3f} | {stage['avg']:.3f} | "
            f"{stage['end']:.3f} | {spill['max']:.3f} | {power_q['avg']:.3f} |"
        )
    ...
    for sta in multi_stair.multi_stair:
        power = _series_stats(sta.history["current_power"])
        flow = _series_stats(sta.history["flow"])
        lines.append(
            f"| {sta.name} | {power['min']:.3f} | {power['max']:.3f} | {power['avg']:.3f} | "
...
def run_simulation(
    ...
    if sim_steps <= 0 or warm_steps <= 0:
        raise ValueError("sim_steps 与 warm_steps 必须为正整数。")
    flow_low, flow_high = _validate_range("flow_range", flow_range[0], flow_range[1])
    power_low, power_high = _validate_range("power_range", power_range[0], power_range[1])

    t0 = time.perf_counter()
    out = _configure_output_dir(output_dir)
    _validate_configs()
    print(f"输出目录: {out}")

    n_total = sim_steps + warm_steps

    flows_in, power_cmd = generate_input_signals(
        sim_steps, warm_steps, flow_seed, power_seed,
        (flow_low, flow_high), (power_low, power_high)
    )

    if make_plots:
        NormalizedSignal(n_total, [1200, 300, 770, 6000], seed=flow_seed).scale(flow_low, flow_high).signal_plot(name="大渡河_上游来流Q(m^3/s)")
        NormalizedSignal(n_total, [1200, 300, 770, 6000], seed=power_seed).scale(power_low, power_high).signal_plot(name="大渡河_电网调度P(MW)")

    multi_river = RiverArray(
        1,
        "大渡河_水力",
        FLOW_CONFIGS,
        max(sim_steps, warm_steps),
        CAPA_LOC,
    )
    ...
    min_p_path = os.path.join(out, "dispatch_min_p_v47.json")
    with open(min_p_path, "w", encoding="utf-8") as f:
        json.dump(dispatch_min_p, f, ensure_ascii=False, indent=2)
    ...
    formal_results_csv = _export_formal_results_csv(
        output_dir=out,
        flows_in=flows_in,
        power_cmd=power_cmd,
    ...
    stage_summary = _stage_zone_summary(multi_reservoir)
    report_path = _export_simulation_report_md(
        output_dir=out,
        sim_steps=sim_steps,
        warm_steps=warm_steps,
        flow_seed=flow_seed,
    ...
    summary_path = _export_run_summary_json(
        output_dir=out,
        sim_steps=sim_steps,
        warm_steps=warm_steps,
        flow_seed=flow_seed,
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description=f"梯级水电站电调-水调联合仿真 V{__version__}")
    parser.add_argument("--sim-steps", type=int, default=720, help="正式仿真步数（分钟）")
    parser.add_argument("--warm-steps", type=int, default=720, help="预热步数（分钟）")
    parser.add_argument("--output-dir", type=str, default=None, help="输出目录")
    parser.add_argument("--progress-interval", type=int, default=100, help="进度打印间隔")
    parser.add_argument("--no-plots", action="store_true", help="不生成图像")
    parser.add_argument("--flow-seed", type=int, default=555, help="上游来流信号随机种子")
    parser.add_argument("--power-seed", type=int, default=123, help="电网调度信号随机种子")
    parser.add_argument("--flow-range", type=float, nargs=2, default=(1700.0, 3300.0), metavar=("LOW", "HIGH"), help="上游来流缩放区间")
    parser.add_argument("--power-range", type=float, nargs=2, default=(1500.0, 4800.0), metavar=("LOW", "HIGH"), help="总调度指令缩放区间")
    return parser
...
def main() -> None:
    ...
    run_simulation(
        ...
        power_range=tuple(args.power_range),
    )
